...
export default abstract class BaseStrapiClient {
  ...
  public async fetchOne<
    ...
    TParams extends AppLocalizedParams,
  >(
    uid: TContentTypeUID,
  ...
  >(
    ...
    const firstPage: APIResponseCollection = await this.fetchAPI(
      path,
  ...
  >(
    ...
    const path = this.getStrapiApiPathByUId(uid)
    const response: APIResponseCollection = await this.fetchAPI(
      path,
      mergedParams,
      requestInit,
      options
